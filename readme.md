...
- fully rewritten in TypeScript with a `src/` → `dist/` build (declarations generated by `tsc`)
- test suite migrated to `ts-jest` (unit, mocked, and live-network suites)
...
The library is authored in TypeScript under `src/` and compiled to CommonJS in `dist/`.

```bash
npm run build      # compile src/ → dist/ (+ .d.ts declarations)
npm run typecheck  # type-check sources and tests without emitting
npm test           # typecheck + run the full jest suite
npm run test:unit  # deterministic unit + mocked suites only (offline)
npm run test:live  # live-network integration suite (requires outbound access)
```
